...
class ExperimentLogger:
    ...
    def save_algorithm_result(
        ...
        row = {
            ...
            'mean_omat': (f"{metrics['mean_omat']:.4f}"
                          if metrics and metrics.get('mean_omat') is not None else ''),
            'std_omat': (f"{metrics['std_omat']:.4f}"
                         if metrics and metrics.get('std_omat') is not None else ''),
            'mean_ess': (f"{metrics['mean_ess']:.2f}"
                         if metrics and metrics.get('mean_ess') is not None else ''),
            'mean_resamples': (f"{metrics['mean_resamples']:.1f}"
                               if metrics and metrics.get('mean_resamples') is not None else ''),
