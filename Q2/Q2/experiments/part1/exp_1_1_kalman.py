"""Kalman Filter numerical stability comparison (TensorFlow)."""
import os
import sys
import time
import numpy as np
...
def run_method(params, ys, xs, joseph):
    """Run KF and return metrics: mse, cond, runtime."""
    ssm = LinearGaussianSSM(params['A'], params['B'], params['C'],
                            params['D'], params['Sigma'])
    ys_tf = tf.constant(ys, dtype=DTYPE)

    result = {'failed': False}
    try:
        kf = KalmanFilter(joseph=joseph)

        t0 = time.perf_counter()
        filt_result = kf.filter(ssm, ys_tf)
        runtime = time.perf_counter() - t0

        m_filt = filt_result.m_filt.numpy()
        cond_nums = filt_result.diagnostics['cond_nums'].numpy()
        max_cond = np.max(cond_nums)

        # Check failure: NaN, Inf, or kappa=inf (P collapsed)
        P_filt = filt_result.P_filt.numpy()
        if np.any(np.isnan(P_filt)) or np.any(np.isinf(P_filt)) or np.isinf(max_cond):
            result['failed'], result['reason'] = True, 'P collapsed (kappa=inf)'
            return result

        mse = np.mean((xs - m_filt) ** 2)
...
def run_all(systems, T, seed):
    ...
    for name, params in systems.items():
        ...
        for joseph, label in METHODS:
            results[name][label] = run_method(params, ys, xs, joseph)
    return results
...
def print_table(results, title):
    ...
    print(f"\n{title}")
    print("-" * 70)
    print(f"{'System':<12} {'Method':<10} {'MSE':<12} {'log10(kappa)':<14} {'Runtime(ms)':<12} {'Status'}")
    print("-" * 70)
    for name, res in results.items():
        for joseph, label in METHODS:
            ...
            if r['failed']:
                ...
                print(f"{name:<12} {label:<10} {r['mse']:<12.6f} {log_cond:<14.2f} {r['runtime']:<12.2f} OK")
...
if __name__ == "__main__":
    save_path = os.path.join(os.path.dirname(__file__), '..', '..', 'results', 'part1', 'exp_1_1_kalman')
