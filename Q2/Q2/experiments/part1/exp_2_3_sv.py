"""Stochastic Volatility flow filter comparison (TensorFlow)."""
import os
import sys
import time
import json
...
from src.flows import EDHFlow, LEDHFlow, RKHSFlow
from src.flows.edh import compute_edh_matrices
from src.flows.ledh import compute_ledh_matrices

DTYPE = tf.float64

# Helper: exponential lambda schedule (not available in TF src)
def exponential_lambda_schedule(n_steps, ratio=1.2):
    """Exponential lambda schedule with cumulative normalisation."""
    raw = np.array([ratio ** i for i in range(n_steps)])
    cumsum = np.cumsum(raw)
    return cumsum / cumsum[-1]


def _make_lambda_positions(n_flow, ratio=1.2):
    """Return [0, lam_1, ..., 1] array suitable for BaseFlow.lambda_schedule."""
    lam = exponential_lambda_schedule(n_flow, ratio=ratio)
    return np.concatenate([[0.0], lam])

# Filter runners (class-based TF API)
...
def run_edh_filter(model, ys, N, n_flow, seed):
    """Run EDH flow filter over all time steps using TF class API."""
    ys_tf = tf.constant(ys.reshape(-1, 1), dtype=DTYPE)
    lam_pos = _make_lambda_positions(n_flow)
    edh = EDHFlow(n_particles=N, n_flow_steps=n_flow,
                  lambda_schedule=lam_pos, filter_type='ekf')
    rng_tf = tf.random.Generator.from_seed(seed)
    result = edh.filter(model, ys_tf, rng=rng_tf)
    return result.m_filt.numpy()


def run_ledh_filter(model, ys, N, n_flow, seed):
    """Run LEDH flow filter over all time steps using TF class API."""
    ys_tf = tf.constant(ys.reshape(-1, 1), dtype=DTYPE)
    lam_pos = _make_lambda_positions(n_flow)
    ledh = LEDHFlow(n_particles=N, n_flow_steps=n_flow,
                    lambda_schedule=lam_pos, filter_type='ekf')
    rng_tf = tf.random.Generator.from_seed(seed)
    result = ledh.filter(model, ys_tf, rng=rng_tf)
    return result.m_filt.numpy()


def run_rkhs_filter(model, ys, N, n_flow, seed):
    """Run RKHS PFF over all time steps using TF class API."""
    ys_tf = tf.constant(ys.reshape(-1, 1), dtype=DTYPE)
    rkhs = RKHSFlow(n_particles=N, n_flow_steps=n_flow,
                       step_size=0.1, loc_radius=4.0,
                       kernel_type='scalar', adaptive_step=True)
    rng_tf = tf.random.Generator.from_seed(seed)
    result = rkhs.filter(model, ys_tf, rng=rng_tf)
    return result.m_filt.numpy()
...
def run_filters(model, xs, ys, N=200, n_flow=15, seed=42):
    ...
    for name, runner in [
        ('EDH', lambda: run_edh_filter(model, ys, N, n_flow, seed)),
        ('LEDH', lambda: run_ledh_filter(model, ys, N, n_flow, seed)),
        ('KernelPFF', lambda: run_rkhs_filter(model, ys, N, n_flow, seed)),
    ]:
        try:
            t0 = time.time()
            m_filt = runner()
            ...
            results[name] = {'mse': np.inf, 'time': time.time() - t0,
                             'status': f'diverged: {e}'}
        except Exception as e:
            results[name] = {'mse': np.inf, 'time': 0,
...
def run_filters_sparse(model, xs, ys, obs_mask, N=200, n_flow=15, seed=42):
    ...
    For missing time steps, only the observed sub-sequence is passed.
    """
    ys_obs = ys[obs_mask]
    results = {}

    for name, runner in [
        ('EDH', lambda: run_edh_filter(model, ys_obs, N, n_flow, seed)),
        ('LEDH', lambda: run_ledh_filter(model, ys_obs, N, n_flow, seed)),
        ('KernelPFF', lambda: run_rkhs_filter(model, ys_obs, N, n_flow, seed)),
    ]:
        try:
            t0 = time.time()
            m_filt_sparse = runner()
            runtime = time.time() - t0
...
# Stability diagnostics (uses TF compute_*_matrices)
def compute_diagnostics(model, N=200, n_steps=15, seed=42):
    ...
    m0_np = model.m0.numpy()
    P0_np = model.P0.numpy()
    particles_np = rng.multivariate_normal(m0_np, P0_np, size=N)
    ...
    y = tf.constant([ys[5]], dtype=DTYPE)
    m = model.m0
    P = model.P0
    H = model.H_jac(m)
    ...
    # EDH diagnostics
    edh_conds, edh_mags = [], []
    pts = tf.constant(particles_np, dtype=DTYPE)
    eta_bar = tf.reduce_mean(pts, axis=0)
    lam = 0.0

    for j in range(n_steps):
        eps = float(lam_schedule[j]) if j == 0 else float(lam_schedule[j] - lam_schedule[j - 1])
        lam_tf = tf.constant(lam, dtype=DTYPE)
        A, b = compute_edh_matrices(m, P, H, model.R, y, lam_tf, eta_bar, model.h)

        A_np = A.numpy()
        cond = np.abs(A_np[0, 0]) if A_np.shape[0] == 1 else np.linalg.cond(A_np)
        edh_conds.append(cond if np.isfinite(cond) else 1.0)

        dx = eps * (pts @ tf.transpose(A) + b[tf.newaxis, :])
        edh_mags.append(float(tf.reduce_mean(tf.abs(dx))))
        ...
        for i in range(min(N, 50)):
            try:
                ...
                A_i, b_i = compute_ledh_matrices(
                    pts[i], m, P, model.h, model.H_jac, model.R, y, lam_tf, R_inv)
                A_i_np = A_i.numpy()
                cond = np.abs(A_i_np[0, 0]) if A_i_np.shape[0] == 1 else np.linalg.cond(A_i_np)
                step_conds.append(cond if np.isfinite(cond) else 1.0)
                dx = eps * (tf.linalg.matvec(A_i, pts[i]) + b_i)
                step_mags.append(float(tf.abs(dx[0])))
            except Exception:
                pass
...
def aggregate_results(all_runs):
    ...
    Returns {method: {mse_mean, mse_std, time_mean, time_std, n_success, n_total}}
    """
...
def exp_nonlinearity(T, N, n_flow, base_seed, n_runs, save_dir):
    ...
    _plot_comparison_with_std(results, exp_scales, 'exp_scale',
                              'Nonlinearity', save_dir, '1_nonlinearity')
    _plot_diagnostics_with_std(diags, exp_scales, 'exp_scale',
                               save_dir, '1_nonlinearity_diag')
...
def exp_observation_sparsity(T, N, n_flow, base_seed, n_runs, save_dir):
    ...
    _plot_comparison_with_std(results, obs_rates, 'obs_rate',
                              'Observation Sparsity', save_dir, '2_sparsity')
    return results
...
def exp_conditioning(T, N, n_flow, base_seed, n_runs, save_dir):
    ...
    _plot_conditioning_with_std(results, conditioning_experiments,
                                save_dir, '3_conditioning')
    _plot_conditioning_diagnostics_with_std(diags, conditioning_experiments,
                                            save_dir, '3_conditioning_diag')
    return results, diags

# Timing comparison helper
def timing_comparison(T, N, n_flow, seed, save_dir):
    """Run a single timing comparison between EDH, LEDH and KernelPFF.

    Prints wall-clock time for each method and saves to timing.json.
    """
    model = SVAdditiveNoise(exp_scale=0.5)
    rng = np.random.default_rng(seed)
    xs, ys = model.simulate(T, rng)
    ...
    with open(os.path.join(save_dir, 'timing.json'), 'w') as f:
        json.dump({k: (v if v is not None else "FAILED") for k, v in timings.items()},
                  f, indent=2)
...
def _plot_diagnostics_with_std(diags, x_vals, x_label, save_dir, prefix):
    ...
    for ax, (metric, metric_std), ylabel in zip(
        ...
        for method, c, marker, ls in [('EDH', 'blue', 'o', '-'),
                                      ('LEDH', 'dodgerblue', 's', '--')]:
...
def _save_raw_results(all_raw_results, save_dir, filename):
    ...
    for key, runs in all_raw_results.items():
        serializable[str(key)] = [
            {method: {k: (v if isinstance(v, str) else float(v))
...
def write_summary(all_results, diags, n_runs, save_path):
    ...
    with open(save_path, 'w') as f:
        f.write(f"Flow Filter Comparison - TensorFlow ({n_runs} runs per condition)\n\n")
        ...
        for cond, res in all_results.items():
            for method in ['EDH', 'LEDH', 'KernelPFF']:
                ...
                f.write(f"{str(cond):<25} {method:<12} {mse_str:>18} "
                        f"{time_str:>18} {success_str:>10}\n")

        f.write(f"\nStability Diagnostics\n")
        f.write(f"{'Condition':<20} {'Method':<8} {'Cond_mean':>20} "
                f"{'Cond_max':>20} {'Mag_mean':>20} {'Mag_max':>20}\n")
        ...
        for cond, d in diags.items():
            for method in ['EDH', 'LEDH']:
                ...
                f.write(f"{str(cond):<20} {method:<8} {cond_mean_str:>20} "
                        f"{cond_max_str:>20} {mag_mean_str:>20} {mag_max_str:>20}\n")
...
def main():
    ...
    try:
        timing_comparison(T, N, n_flow, base_seed, save_dir)
    except Exception as e:
        print(f"Timing comparison failed: {e}")

    # Experiment 1: Nonlinearity
    res1, diag1 = {}, {}
    res2 = {}
    res3, diag3 = {}, {}
    ...
    print("Experiment 2: Observation sparsity ...")
    try:
        res2 = exp_observation_sparsity(T, N, n_flow, base_seed, n_runs, save_dir)
    except Exception as e:
        print(f"  Failed: {e}")

    print("Experiment 3: Conditioning ...")
    try:
    ...
    if all_results:
        write_summary(all_results, diags, n_runs,
